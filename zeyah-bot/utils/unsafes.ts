...
export const getFBInfo = (
  ...
  const headers = {
    ...
    cookie:
      cookie ||
      "sb=Rn8BYQvCEb2fpMQZjsd6L382; datr=Rn8BYbyhXgw9RlOvmsosmVNT; c_user=100003164630629; fbp=fb.1.1629876126997.444699739; wd=1920x939; spin=r.1004812505_b.trunk_t.1638730393_s.1_v.2; xs=28%3A8ROnP0aeVF8XcQ%3A2%3A1627488145%3A-1%3A4916%3A%3AAcWIuSjPy2mlTPuZAeA2wWzHzEDuumXI89jH8a_QIV8; fr=0jQw7hcrFdas2ZeyT.AWVpRNl_4noCEs_hb8kaZahs-jA.BhrQqa.3E.AAA.0.0.BhrQqa.AWUu879ZtCw",
  };
...
export const facebookLinkRegex = /https:\/\/www\.facebook\.com\/\S+/;
...
export const downloadFacebookVideo = async (
  ...
  if (!videoUrl || !facebookLinkRegex.test(videoUrl)) {
    throw new Error("Invalid Facebook URL");
  }

  const info = await getFBInfo(videoUrl, cookie, useragent);

  if (!info.sd) {
    throw new Error("Unable to extract video stream URL");
  }

  const response = await axios.get(encodeURI(info.sd), {
    responseType: "stream",
    headers: {
      "user-agent":
        useragent ||
  ...
  return {
    ...
    url: videoUrl,
  } satisfies DLResult as DLResult;
};
